...
public class Tomcat9EmbeddedServer implements EmbeddedServer {
    final Tomcat tomcat;
    Context context;
    ...
    public Tomcat9EmbeddedServer() {
        ...
        if (tomcat.getHost() instanceof StandardHost) {
            ...
            File appBase = new File("target/tomcat9/webapps");
            if (!appBase.exists()) appBase.mkdirs();
            host.setAppBase(appBase.getAbsolutePath());

            File workDir = new File("target/tomcat9/work");
    ...
    private Context addWebapp(String contextPath, String appBase, ClassLoader loader, boolean mainContext) {
        ...
        if (loader != null) {
            ...
            if (mainContext && ClassLoaderFactoryHolder.getClassLoaderFactory() != null) {
                webappLoader.setLoaderClass("net.unit8.waitt.server.tomcat9.Tomcat9WebappClassLoaderWrapper");
