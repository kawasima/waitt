...
public class Tomcat11EmbeddedServer implements EmbeddedServer {
    final Tomcat tomcat;
    Context context;
    List<WebappDecorator> decorators;

    public Tomcat11EmbeddedServer() {
        tomcat = new Tomcat();
        if (tomcat.getHost() instanceof StandardHost) {
            StandardHost host = (StandardHost) tomcat.getHost();
            host.setUnpackWARs(true);
            File appBase = new File("target/tomcat11/webapps");
            if (!appBase.exists()) appBase.mkdirs();
            host.setAppBase(appBase.getAbsolutePath());

            File workDir = new File("target/tomcat11/work");
    ...
    private Context addWebapp(String contextPath, String appBase, ClassLoader loader, boolean mainContext) {
        ...
        if (loader != null) {
            ...
            if (mainContext && ClassLoaderFactoryHolder.getClassLoaderFactory() != null) {
                webappLoader.setLoaderClass("net.unit8.waitt.server.tomcat11.Tomcat11WebappClassLoaderWrapper");
