 ...
 * @author shimoda
 */
public class Tomcat85EmbeddedServer implements EmbeddedServer {
    final Tomcat tomcat;
    Context context;
    List<WebappDecorator> decorators;

    public Tomcat85EmbeddedServer() {
        tomcat = new Tomcat();
        if (tomcat.getHost() instanceof StandardHost) {
            StandardHost host = (StandardHost) tomcat.getHost();
            host.setUnpackWARs(true);
            File appBase = new File("target/tomcat85/webapps");
            if (!appBase.exists()) appBase.mkdirs();
            host.setAppBase(appBase.getAbsolutePath());

            File workDir = new File("target/tomcat85/work");
